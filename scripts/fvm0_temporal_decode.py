...
def decode(video:Path, manifest_path:Path, output:Path):
 m=json.loads(manifest_path.read_text(encoding='utf8')); c=TemporalConfig.from_manifest(m)
 ...
 actual_width,actual_height=int(cap.get(3)),int(cap.get(4)); actual_fps=float(cap.get(5))
 if (actual_width,actual_height) != (c.width,c.height): raise RuntimeError('video resolution does not match manifest')
 ...
 while cap.read()[0]: actual_frames += 1
 ...
 errors=sum(x['bit_errors'] for x in records); total=len(records)*c.cells_per_frame; warnings=[]
 if actual_frames != len(m['schedule'])*c.block_size: warnings.append('FVM0-T has no synchronization metadata; frame insertion/deletion makes index-based comparison unreliable after synchronization loss.')
 if abs(actual_fps-c.fps)>.01: warnings.append('decoded FPS differs from manifest')
 result={'format':m['format'],'video':{'expected_resolution':f'{c.width}x{c.height}','actual_resolution':f'{actual_width}x{actual_height}','expected_fps':c.fps,'actual_fps':actual_fps,'expected_frames':len(m['schedule'])*c.block_size,'actual_frames':actual_frames,'compared_frames':len(records)},'matrix':{'cell_size':c.cell_size,'rows':c.rows,'columns':c.cols,'cells_per_frame':c.cells_per_frame},'experiment':m,'overall':{'compared_frames':len(records),'total_bits':total,'correct_bits':total-errors,'bit_errors':errors,'ber':errors/total if total else None},'ratios':aggregate_ratios(records,c.cells_per_frame,c.fps,c.block_size),'warnings':warnings}
